...
internal sealed class RivenAppraisalPanel
{
    ...
    public async Task<string> SetupAsync(SocketGuild guild, CancellationToken ct)
    {
        ...
        var embed = new EmbedBuilder().WithTitle("Riven Appraisal Desk").WithColor(new Color(0x8E44AD))
            ...
            .AddField("Screenshot OCR", "Drop one Riven screenshot directly into this channel, use `/rf-riven appraise image:`, or press **Appraise screenshot** for a Discord image link. Local Tesseract compares up to eleven color, grayscale, deskewed, threshold and card-region passes against known weapons/stats. Generated affixes cross-check positives; shared scaling tests every possible fourth attribute; footer consensus protects MR/rerolls from stray digits. Mod rank stays editable because the card shows pips, not reliable rank text. Implausible values are rejected. Faction multipliers above x1 become positive bonuses; values below x1 become negatives (for example x0.6 = −40%). Always review the editable draft. No image or recognized text is saved, and no generative-AI service is used.")
