...
The owner-only **Sell sets** button chooses between complete-set listings and separate component listings. With **Sell sets ON** (the default), the bot reserves exactly the components needed for each complete set, lists that set, and lists any extra copies of its components separately. With **Sell sets OFF**, it retires set listings and treats components as separate stock. The independent **Protect completions** button defaults to ON: after complete sets are allocated, it reserves one set's owned components when exactly one required component type is still missing. Copies beyond that reservation remain eligible, and parts from sets missing two or more component types remain eligible. Turn protection OFF to sell near-complete-set parts too. Both choices are saved locally; when automatic listing is enabled, changing either choice can delete or replace existing market orders. Review the resulting board before enabling automatic listing on a new installation.
