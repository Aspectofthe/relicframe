...
| Baro investment assistant | Partial | `#baro-investments` reads official current stock, Ducat/Credit costs, separate R0 and max-rank 30-day medians/live asks, and sorts by R0 sales/day or return per Ducat/Credit. It records observed visits for post-departure comparisons. Net R0 profit requires user-supplied opportunity costs for both resources; max-rank Endo/ranking cost and future resale price are not invented. |
| Syndicate standing converter | Implemented | `#standing-profit` maps the six main Syndicates' WFCD/wiki vendor costs and rank requirements to marketplace mods, weapons and components. Filters by Syndicate, sorts by activity-weighted return/p25k standing/sales per day, and converts an editable standing budget into whole-item quantities and remaining standing. Uses fresh R0 prices capped by 30-day medians and minimum reported activity; no inferred account standing or automatic purchases. |
