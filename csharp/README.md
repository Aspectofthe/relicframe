...
Combined elemental and new combined-stat names are parsed as positive attributes and preserved in evidence. Their final roll bases and grades intentionally stay unavailable until the live rules publish them. The same parser, pricing core, marketplace catalog discovery and Tesseract pipeline are used by Windows and Linux; there is no reduced Linux-only Riven path.
