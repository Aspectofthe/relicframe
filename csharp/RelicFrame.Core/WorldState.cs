...
public static class WorldRender
{
    ...
    public static WorldBoard[] Build(string key, WorldSnapshot data, IReadOnlyList<ArbitrationEntry> schedule)
    {
        ...
        [
            ...
            Board(key, "Personal market, OCR and safety", "**#personal-market** lists owned Prime stock outside THE LIST filters. It excludes your own order IDs when undercutting competitors. Sync allows 25 writes per pass; Pause stops writes. `EE.log` confirms successful itemized trades and suppresses sold stock before AlecaFrame refreshes; inventory changes wake the board. Order reductions also suppress stale stock; AlecaFrame supplies confirmed gross. A vanished order is held from relisting until a confirmed trade or fresh reduced inventory resolves it; disappearance alone is not a sale. Screen OCR reads Trade Chat, not trade success.\n\nWindows uses `run-bot.cmd` with encrypted credentials; Linux uses `run-bot.sh` with mode-600 credentials. **#riven-appraisal** uses local OCR before an editable draft; it stores no image/text and makes no AI request. Never share credentials.", 0x00AEEF)
        ];
        if (key == "world-cycles")
        {
