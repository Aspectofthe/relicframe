...
public static class WorldRender
{
    ...
    public static WorldBoard[] Build(string key, WorldSnapshot data, IReadOnlyList<ArbitrationEntry> schedule)
    {
        ...
        [
            Board(key, "RelicFrame — start here", "RelicFrame tracks Warframe world activity and public market asks. `/rf-status` is the fast health check. Managers use `/rf-world setup` to repair channels/roles. Use the buttons in #role-pings to opt yourself into notifications. Pings baseline silently on first observation and fire only for newly seen activity.", 0x5865F2),
            Board(key, "Relics and the live panel", "`/rf-panel` controls the persistent ranking board. It defaults to **Radiant**, **Best Overall**, and the best available seller scope. `config` changes refinement, ranking, seller scope, vault state, ROI, cost, reward and guaranteed-profit filters; use `-1` to clear numeric filters. `stop` is its one-minute render kill switch. `/rf-relics stop` separately stops API fetching.\n\n`/rf-relics list/detail/find/odds/compare/buyn` provide on-demand calculations. Expected value is an average—not a promised outcome. Green requires every reward outcome to clear the listed cost. Yellow has positive expected profit but can lose. Red has non-positive expected profit. White means required price data is missing. Buy-N reports exact probabilities when safe and explicitly labels Monte Carlo estimates.", 0x8E44AD),
            Board(key, "Riven resale candidates", "Run `/rf-riven refresh`, watch progress in `/rf-status`, then use `/rf-riven flips`. Candidates must satisfy the supplied desired-roll rules, have a harmless negative, and have at least three comparable asks. Projected resale uses 90% of a weighted peer median and is capped by Digital Extremes' completed-trade range where available. Warframe.market listings are asks, its search responses are capped, and a candidate is **not a guaranteed sale**. The listing/profile links and `/w` text are manual conveniences; the bot never buys or messages anyone.", 0xA855F7),
            Board(key, "Companion breeding estimates", "`/rf-companion` currently accepts manually identified natural species, breed, pattern, build, rarity and colors. It weighs current evidence above historical evidence and distinguishes confirmed sales, listings, mentions and appraisals. Estimates are advisory platinum ranges, not guaranteed sales. Screenshots are not automatically classified in the C# preview; use natural colors/patterns rather than marketplace cosmetics, and do not present an uncertain image guess as confirmed.", 0x27AE60),
            Board(key, "Live feeds, controls and safety", "WARFRAME LIVE contains cycles, news, alerts, Sortie, Archon, Steel Path, weekly missions, Archimedea, vendors, bounties, normal/Steel fissures, Void Storms, invasions, Arbitration and Cascade. Fissure rows show `Lvl <grade> tier`; Cascade separates normal from Steel Path. When translated fissures are stale, a fresh official Digital Extremes snapshot repairs them.\n\nManage Server is required for setup/start/stop/manual refresh controls. Market calls share a five-per-second budget, workers are cancellable, and last successful data is retained on failures. Never paste bot/API tokens into chat or commit them. Use a separate test bot until every migration gate and live soak test passes.", 0xE67E22)
        ];
        if (key == "world-cycles")
        {
