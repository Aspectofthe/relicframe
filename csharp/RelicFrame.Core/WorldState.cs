...
public static class WorldRender
{
    ...
    public static WorldBoard[] Build(string key, WorldSnapshot data, IReadOnlyList<ArbitrationEntry> schedule)
    {
        ...
        [
            Board(key, "RelicFrame — complete starting guide", "**Health:** `/rf-status` bypasses calculations and full market snapshots. Its loading ETA follows market throughput and the active Riven scan stage, then changes to ready. Three command workers keep unrelated requests moving; buttons acknowledge immediately, then do network work outside the gateway task.\n\n**Setup:** `/rf-world setup` repairs WARFRAME LIVE and roles; `/rf-panel setup` repairs THE LIST. Setup replaces prior bot guides/boards without deleting user messages.\n\n**Data rule:** listings and Trade Chat posts are asks, not sales. Stale, partial, inferred, withdrawn and confirmed evidence stay separate; failures retain the last successful snapshot.", 0x5865F2),
            ...
            Board(key, "Riven appraisal", "Use **#riven-appraisal** or `/rf-riven appraise` with 2–3 legal positives and an optional negative. Local OCR creates an editable draft; F–S grades measure roll height, not usefulness. Results show desired alternatives, supported negatives, conditions and source. Outliers are filtered; only **Record sold** confirms a sale.\n\nThe 418-family guide is primary and gaps are labelled fallback. Breakpoints apply to appraisal and flips; low-disposition/shotgun CC stays conservative, CD assumes a CD build, and −Finisher is rejected for sensitive melee.", 0xA855F7),
            ...
            Board(key, "Personal market, OCR and safety", "The loader fetches Vaulted and Unvaulted prices once. THE LIST shows qualifying Vaulted relics; **#personal-market** independently uses all mapped owned Prime parts. It lists 1–3p below the lowest valid **in-game** WTS seller without crossing your floor. Sync allows 25 writes per 15 minutes; Pause stops writes. Managed orders that become ineligible are deleted. Managed market-order reductions/closures are recorded as sales and deducted from stale inventory until a later inventory refresh reflects them; inventory changes alone are not called sold.\n\nWindows uses `run-bot.cmd` with a Windows-user-encrypted credential; Linux uses `run-bot.sh` with owner-only mode-600 credential files. Both live in the ignored runtime folder and support a forget-credentials option. In **#riven-appraisal**, local image OCR checks affixes and stat ranges before an editable draft; other channels are ignored. It stores no image/text and makes no OpenAI, LLM, or generative-AI request. Companion features are disabled. Never share credentials.", 0x00AEEF)
        ];
        if (key == "world-cycles")
        {
